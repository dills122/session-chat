...
Candidate v3 introduced the narrower tuple, which candidate v4 retains while
adding build-bound compiler provenance under ADR 0034:
...
`protocol=l2-evidence-candidate-v4`. Frozen v3 and v2 fixtures retain negative
compatibility cases. Hosted attestation can authenticate exact v4
...
CI may continue collecting and attesting v4 recovery candidates. Consumers can
...
The redaction contract changed incompatibly from v2 to v3. ADR 0034 later
changed compiler provenance incompatibly from v3 to v4. Callers no longer
construct an `L2EvidenceChannels` value or choose promotion surfaces.

## Evidence and limits

Tests reject v2/v3, retire every v2 promotion entry point, prove the arbitrary
channel constructor is unavailable, require the exact v4 demotion fields, and
