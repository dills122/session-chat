...
| `D9-RESEARCH-RECONCILE` | Merged fault-research integration | companion fault packet | Active | Persist the per-row attempt ceiling, reject pre-reopen process-scoped leases, bind schema v2 to `user_version`, use an exclusive migration, and read back the retained journal/synchronous configuration. The portable child-kill/VFS suite, full structural fingerprint, immutable encrypted v1 artifact, and rollback anchor remain separate L2 gates. |
