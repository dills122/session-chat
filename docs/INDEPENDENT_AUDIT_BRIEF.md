...
| Inviter reservation, MLS state, and Welcome outbox creation are atomic and delivery resumes after close/reopen | Implemented and tested in-process and across graceful Alice process exit | The real capability path recovers an ambiguous commit, reloads the exact identity/group, leases the committed Welcome, and delivers it once. ADR 0021 repeats the positive reload in a fresh Alice process; bounded join-writer application-kill evidence is listed below, while durable approval/replay reload, Welcome-delivery crash recovery, and power-loss recovery remain unimplemented |
