...
- `sessionctl`: a headless two-client composition covering protected capability
  ...
  records remain non-public; the retained L2-8 gate promotes only complete
  clean-runner aggregates after exact provenance/artifact binding and bounded
  multi-surface canary scans, with portable passage remaining per-revision
