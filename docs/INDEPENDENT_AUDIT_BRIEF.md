...
- `sessionctl`: a headless two-client composition covering protected capability
  ...
  matrices emit explicitly unsigned candidate v4 bundles with in-process Git
  status, build-bound compiler identity/digest, execution-time binary/artifact
  binding, and bounded case-surface secret/canary scans. Candidate v4 marks
  capture completeness unproved and redaction unverified under ADR 0030.
