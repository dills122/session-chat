...
| Durable MLS identity storage is secret-safe and bound to one exact group | Implemented and tested | The 141-byte v1 record crosses the public trait only in an opaque non-`Clone`, non-`Debug`/non-`Display` type; schema v4 stores an exact group binding, and create/load/join reject cross-group use while the frozen RFC 8032 and schema-v3 fixtures preserve compatibility |
