...
- `storage-sqlcipher-fault-vfs`: a publish-disabled, non-default named SQLite
  delegator used only by checked L2 builds to retain bounded path-free operation
  evidence, inject actual FULL/extended-IOERR codes, and block at selected
  rollback-journal/main-database commit-window operations;
- `sessionctl`: a headless two-client composition covering protected capability
  ...
  aggregates emit canonical per-case bundles after actual compiler/GitHub-run/
  runner-tuple and artifact binding plus bounded multi-surface secret/canary
  scans, with portable passage remaining per-revision
...
| One inviter reservation/consumption shadow is atomically composed with MLS and Welcome-outbox persistence | Implemented and tested in the SQLCipher-backed headless paths | `sessionctl` and `storage-sqlcipher` cover durable opening issuance, pre-membership restart abandonment, replay retention, rollback, ambiguous commit recovery, exact reload, and delivery resumption; rollback resistance remains unimplemented |
...
| Pending approval has a provider-neutral observation and decision seam | Implemented and tested | ADR 0022 and `session-admission` expose only redacted, non-authorizing context; the capability provider retains exact proof, KeyPackage, and reservations |
...
| Approval, invitation state, replay state, MLS Add, and Welcome outbox are independently recoverable as one durable product transaction | Implemented and tested as a bounded laboratory transaction | SQLCipher reloads bounded non-authorizing approval/replay shadows and reconciles exact membership transaction outcomes without repeating Add. Both headless capability paths transfer the exact live provider-applied Add through that owner; rollback resistance and production key custody remain absent |
...
| Request ID and nonce are replay-reserved within one invitation generation | Implemented and tested | Bounded in-memory reservations protect the live provider path, while the composed SQLCipher owner persists exact request ID, nonce, and fingerprint replay state through generation expiry and fresh-process recovery. This does not establish rollback resistance |
...
| Inviter reservation, MLS state, and Welcome outbox creation are atomic and delivery resumes after close/reopen | Implemented and tested in-process and across graceful Alice process exit | The real capability path uses durable authorization, recovers an ambiguous commit, reloads the exact identity/group, leases the committed Welcome, and delivers it once. ADR 0021 repeats the positive reload in a fresh Alice process; Welcome-delivery crash recovery and power-loss recovery remain unimplemented |
...
| SQLite-visible FULL/extended-IOERR failures and named commit-window process kills reopen to one exact complete inviter/joiner state | Implemented and locally tested; public promotion gate retained, portable passage is per-revision CI evidence | `sessionctl` derives every target ordinal from a fresh named-VFS baseline, rejects incomplete return-code/pause coverage, disables faults or confirms child termination before a fresh verifier, and accepts only I0/I1 or J0/J1 with exact retry. Raw L2-5 records prohibit publication; sealed complete aggregates emit canonical per-case L2-8 bundles only after actual compiler/GitHub-run/runner-tuple and artifact binding plus secret/canary scans. Power loss, filesystem behavior, rollback resistance, and production durability remain unimplemented or unclaimed. |
| Every baseline-observed inviter/joiner application checkpoint reopens to exact I0/I1 or J0/J1 state | Implemented and locally tested; public promotion gate retained, portable passage is per-revision CI evidence | `sessionctl` discovers each clean transaction trace, kills the direct writer while the target checkpoint remains unacknowledged, rejects missing/duplicate sweep coverage, freshly verifies exact SQL/MLS state, and requires mutation-free exact retry. L2-8 applies the same promotion/provenance/redaction gate. Retained-KeyPackage and conflicting-retry defects fail closed. Power loss, filesystem behavior, rollback resistance, and production durability remain unimplemented or unclaimed. |
